...
Public Subnet에 직접 노출되어 있던 EC2 접근 구조를 ALB 기반 구조로 변경
- Free Tier 계정 제한으로 Route 53 도메인 등록이 제한되어 HTTPS 커스텀 도메인 연결은 진행하지 못했습니다.
  현재는 ALB 기본 DNS를 통해 HTTP 접속 검증까지 완료하였습니다.

```text
1. 기존 Public EC2 직접 접근 차단 및 ALB Health Check 성공 캡처
2. ALB 구성 확인
3. Target Group Healthy 상태 확인
```
![기존 Public EC2 직접 접근 차단 및 ALB Health Check 성공](docs/images/lv5-access-check.png)
![ALB 구성 확인](docs/images/lv5-alb-detail.png)
![Target Group Healthy 상태 확인](docs/images/lv5-target-group-healthy.png)

<br>

### LV 6. CloudFront CDN 이미지 조회

- S3 버킷을 Origin으로 하는 CloudFront 배포를 생성하고, CloudFront 기본 도메인을 통해 프로필 이미지가 정상 조회되는 것을 확인
- Amazon CloudFront 이미지 URL: `https://d3v1a9a8xx29do.cloudfront.net/uploads/members/1/1975bc4c-6c5d-426c-a1c5-3b238aa0243a-ChatGPT%20Image%202026%EB%85%84%205%EC%9B%94%2025%EC%9D%BC%20%EC%98%A4%ED%9B%84%2008_21_28.png`

![CloudFront 이미지 조회 성공 화면](docs/images/lv6-cloudfront-distribution.png)